...
def test(test_path,txt_path,gt_path,modelsave_path):
    score_correct = []
    disimg_correct1 = []
    input_dataset = TestDataset(test_path,txt_path,gt_path)
    input_dataloader = DataLoader(input_dataset, batch_size=8, shuffle=True, num_workers=4)
    net = ResNet50().cuda()
    net.load_state_dict(torch.load(os.path.join(modelsave_path)),False)
    net.eval()
    ...
    status = "disimg_mse_acc:{},score_acc:{}".format(disimg_acc1, score_acc)
    print(status)
